...
<div class="row">
    <div class="col-sm-12 col-md-9">
        <div class="news-list">
            <?php if ($page ==1) { ?>
            <h4 style="text-align: left;padding-bottom: 0;padding-top: 10px;" class="vc_custom_heading">{{ __('lang._NEWS_TopNews') }}</h4>

            <div id="{{ $AllNews[0]->post_type }}-{{ $AllNews[0]->ID }}"
                 class="item img-top {{ $AllNews[0]->post_type }}-{{ $AllNews[0]->ID }} post type-post status-publish format-standard has-post-thumbnail sticky hentry category-uncategorized">
                <div class="img-wrap">
                    <a href="{{ url('/') }}/News/NewsWithTimeLine/{{ $AllNews[0]->date_route }}/{{ $AllNews[0]->post_name }}/">
                        <img src="{{ config('app.cloudfront.content') }}/uploads/{{ $AllNews[0]->img }}" alt="{{ $AllNews[0]->post_title }}">
                    </a>
                </div>
                <div class="info">
                    <a href="{{ url('/') }}/News/NewsWithTimeLine/{{ $AllNews[0]->date_route }}/{{ $AllNews[0]->post_name }}/"
                       class="name">{{ $AllNews[0]->post_title }}</a>

                    <div class="wrap">
                        <a href="{{ url('/') }}/{{ $news_lang }}News/NewsWithTimeLine/{{ $AllNews[0]->date_route }}/">{{ $AllNews[0]->format_date }}</a>
                    </div>
                    <div class="clear"></div>
            ...
            <?php for($i=($page==1?1:0);$i<count($AllNews);$i++) { ?>
            <div id="{{ $AllNews[$i]->post_type }}-{{ $AllNews[$i]->ID }}"
                 class="item img-top {{ $AllNews[$i]->post_type }}-{{ $AllNews[$i]->ID }} post type-post status-publish format-standard has-post-thumbnail hentry category-uncategorized">
                <div class="img-wrap">
                    <a href="{{ url('/') }}/News/NewsWithTimeLine/{{ $AllNews[$i]->date_route }}/{{ $AllNews[$i]->post_name }}/">
                        <img src="{{ config('app.cloudfront.content') }}/uploads/{{ $AllNews[$i]->img }}" alt="{{ $AllNews[$i]->post_title }}">
                    </a>
                </div>
                <div class="info">
                    <a href="{{ url('/') }}/News/NewsWithTimeLine/{{ $AllNews[$i]->date_route }}/{{ $AllNews[$i]->post_name }}/"
                       class="name">{{ $AllNews[$i]->post_title }}</a>

                    <div class="wrap">
                        <a href="{{ url('/') }}/{{ $news_lang }}News/NewsWithTimeLine/{{ $AllNews[$i]->date_route }}/">{{ $AllNews[$i]->format_date }}</a>
                    </div>
                    <div class="clear"></div>
