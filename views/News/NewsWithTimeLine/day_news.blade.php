...
<div class="row">
    <div class="col-sm-12 col-md-9">
        <div class="news-list">
            ...
            <?php for($i = 0;$i < count($day_news);$i++) { ?>
            <div id="{{ $day_news[$i]->post_type }}-{{ $day_news[$i]->ID }}"
                 class="item img-top {{ $day_news[$i]->post_type }}-{{ $day_news[$i]->ID }} post type-post status-publish format-standard has-post-thumbnail hentry category-uncategorized">
                <div class="img-wrap">
                    <a href="{{ url('/') }}/News/NewsWithTimeLine/{{ $day_news[$i]->date_route }}/{{ $day_news[$i]->post_name }}/">
                        <img src="{{ config('app.cloudfront.content') }}/uploads/{{ $day_news[$i]->img }}"
                             alt="{{ $day_news[$i]->post_title }}">
                    </a>
                </div>
                <div class="info">
                    <a href="{{ url('/') }}/News/NewsWithTimeLine/{{ $day_news[$i]->date_route }}/{{ $day_news[$i]->post_name }}/"
                       class="name">{{ $day_news[$i]->post_title }}</a>

                    <div class="wrap">
                        <a href="{{ url('/') }}/{{ $news_lang }}News/NewsWithTimeLine/{{ $day_news[$i]->date_route }}/">{{ $day_news[$i]->format_date }}</a>
                    </div>
                    <div class="clear"></div>
